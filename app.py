...
def save_response(proyecto_id, name, p10, p90, p50_percentil, comentarios=""):
    """Guarda una respuesta en el CSV (sin datos personales).

    Guardamos: `Timestamp`, `Proyecto`, `P10`, `P90`, `P50_percentil` y `Comentarios`.
    """
    new_data = {
...
for idx, proyecto in enumerate(proyectos_activos):
    with tabs[idx]:
        ...
        with st.form(f"form_{proyecto['id']}"):
            ...
            with col1:
                st.markdown("#### Percentil Bajo (Mejor Caso)")
            ...
            with col2:
                st.markdown("#### Percentil Alto (Peor Caso)")
            ...
            if submitted:
                ...
                if not name or str(name).strip() == "":
                    ...
                    try:
                        ...
                        # P10 no menor que límite inferior y no mayor que P50
                        if p10f < lower_limit or p10f > p50_project_val:
                            st.error(f"⚠️ P10 debe ser ≥ {lower_limit} y ≤ P50 ({p50_project_val})")
                        # P90 no menor que P50 y no mayor que límite superior
                        elif p90f < p50_project_val or p90f > upper_limit:
                            st.error(f"⚠️ P90 debe ser ≥ P50 ({p50_project_val}) y ≤ límite superior ({upper_limit})")
                        else:
                            # Guardar P10, P90 y el percentil que el usuario asignó al Valor Esperado
                            saved = save_response(
                                proyecto['id'],
                                str(name).strip(),
                                round(p10f, 1),
...
with tabs[-1]:
    ...
    if not todos_proyectos:
        ...
        if len(df_proyecto) == 0:
            ...
            st.divider()
                    # Gráfico 1: Histograma de P10
            with col1:
                st.write("#### Percentil Bajo (P10)")
            ...
            # Gráfico 2: Histograma de P90
            with col2:
                st.write("#### Percentil Alto (P90)")
        ...
        # ===== Estadísticas del mapeo de P50 (percentil) =====
            ...
            # Calcular promedios (P10, P50 fijo del proyecto, P90)
            promedios = {
            ...
            fig3.add_trace(go.Bar(
                ...
                marker_color=['#2ECC71', '#FF6B6B', '#F39C12'],
        ...
        if st.session_state.confirm_delete:
            ...
            with colc1:
                if st.button("⚠️ CONFIRMAR BORRADO (IRREVERSIBLE)", key="confirm_delete_root", use_container_width=True):
                    empty = pd.DataFrame(columns=['Timestamp','Proyecto','Nombre','P10','P90','P50_percentil','Comentarios'])
